    ...
    for id, network in ipairs(tech_api.energy.networks) do
      ...
      for pos, device in pairs(network.devices) do
        if device.type == 'storage' then
          if not storage_rates[pos] then
            storage_rates[pos] = {}
          end
          storage_rates[pos][device.def_name] = 0
        end
      end
      ...
      for pos, device in pairs(network.devices) do
        if device.type == 'user' then
          ...
          if device.callback_countdown == 0 then
            ...
            -- be true (callback(pos, -> device.max_rate <-))
            local new_rate, next_callback = device.callback(pos, device.max_rate)
        ...
        elseif device.type == 'storage' then
          ...
          local nd_dev = tech_api.utils.nodestore.data[tech_api.utils.misc.hash_vector(pos)]
      ...
      for pos, device in pairs(network.devices) do
        if device.type == 'provider' then
          device.callback_countdown = device.callback_countdown - 1
          if device.callback_countdown == 0 then
            local new_rate, next_callback = device.callback(pos, request)
            device.current_rate = new_rate
            device.callback_countdown = next_callback
          end
          request = request - device.current_rate
      ...
      if request > available then
        for pos, device in pairs(network.devices) do
          if device.type == 'storage' then
            ...
            storage_rates[pos][device.def_name] = storage_rates[pos][device.def_name] - ask_from_storage
          end
        end
      end
      ...
      for pos, device in pairs(network.devices) do
        if device.type == 'user' then
          ...
          if available < device.current_rate then
            ...
            local new_rate, next_callback = device.callback(pos, math.min(available, device.max_rate))
            device.current_rate = new_rate
            device.callback_countdown = next_callback
          end
      ...
      for pos, device in pairs(network.devices) do
        if device.type == 'storage' then
          ...
          -- storage_rates table we prepared before
          storage_rates[pos][device.def_name] = storage_rates[pos][device.def_name] + put_in_storage

          -- handle callbacks (for storages is only used to report content and
          -- capacity)
          device.callback_countdown = device.callback_countdown - 1
          if device.callback_countdown == 0 then
            local next_callback = device.callback(pos, nd_def.content, device.capacity, storage_rates[pos][device.def_name])
            device.callback_countdown = next_callback
          end
        end
      end
